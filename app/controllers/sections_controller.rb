class SectionsController < ItemsController
  ...
  def apply_to_checked_items
    if params[:action_to_perform] == 'Delete'    
        Section.destroy(params[:delchk].keys) if params[:delchk] && params[:delchk].keys.size > 0
    end 
  ...
  def replace_column_value
    ...
    Section.update_all("#{params[:field]} = '#{params[:new_value].strip}'", where_clause)
    redirect_to :back
  end
  ...
  def delete_matching_rows
    ...
    Section.delete_all(where_clause)
    redirect_to :back
  end
  ...
  def get_related
    ...
    if params[:relname].to_s.underscore == 'property_attrs' then
      render :xml => @section.property_attrs.find(:all)
      return
    end
  ...
  def create
    sanitize_params(:section)
    @new_obj = @section = Section.new(params[:section])
    ...
    if ! ok_to_add_or_update?
      ...
      respond_to do |format|
        if @section.save
          ...
          format.html {
            flash[:notice] = "#{@section.iname} was successfully created."
            redirected_to_aggregator = update_aggregator(@section,params)
            if ( ! redirected_to_aggregator ) && session[:post_create_redirect]
                url = session[:post_create_redirect] + "?section_id=" + @section.id.to_s + "&section_ids[]=" + @section.id.to_s 
                redirect_to url
                session[:post_create_redirect] = nil
            else 
                redirect_to :id => @section.id, :action => (params[:commit].to_s.include?("Another") ? 'new' : 'edit') if ! redirected_to_aggregator 
            end
          }
        else
          format.xml  { render :xml => @section.errors, :status => :unprocessable_entity }
          format.html {
            flash[:notice] = 'Section could not be created. See errors.'
            render :action => 'new'
          }
        end
      end
  ...
  def update
    @updated_obj = @section = Section.find(params[:id])
    sanitize_params(:section)
    ...
    if ! ok_to_add_or_update?
      ...
      @ft_subject = "Section"
      @ft_activity = "Edit"
      redirect_to :action => 'edit', :id => @section and return
    end 
  ...
  def ok_to_add_or_update?
    if params[:section][:name].to_s.empty?
  ...
  def upload_csv
    ...
    #fn = "#{RAILS_ROOT}/uploads/section_data.csv"
    #File.open(fn, "wb") { |f| f.write(params[:csv_file].read) }
    #Section.load_csv_file(fn) if Section.respond_to? :load_csv_file

    Section.load_csv_str(params[:csv_file].read) if Section.respond_to? :load_csv_str
  ...
  def download_csv
    if params[:field_list].nil? || @user_is_admin
      attr_names = Section.value_attrs
    ...
    send_data csv_string, :filename => 'section_data.csv', :type => 'text/csv'
  end 
  ...
  def run_query
    ...
    @query = Section.query_specs[params[:query]]
    ...
    @count = Section.count_by_sql("SELECT COUNT(id) FROM (#{@base_sql}) AS row_data")
    
    @sql = @base_sql + (@sort_fields ? " ORDER BY #{@sort_fields}" : "") + (@page_clause ? " #{@page_clause}" : "") 
    @rows = Section.find_by_sql(@sql)

    @cols = @query[:cols] || (@rows.size > 0 && @rows[0].attributes.keys)

    @link_params = { :controller => 'sections', :action => 'show' }
  ...
  def prepare_list
    ...
    @ft_subject  = ( @ft_subject || "Section".pluralize )
    ...
    adjust_sorting_and_filtering_params(Section,params)
