...
class Attribute < Item
  ...
  def move_targets()
   ...
   def get_children
    ...
    def self.value_attrs
      ["name","created_at","updated_at","sort_code","value"]
    end
 ...
 public
  ...
  def next_at_top
    item = Attribute.find(:first, :order => FtUtils.quoted_name('name'), :conditions => ["name > ? AND owner_info_node_id is null", self.name] )
    item = Attribute.find(:first, :order => FtUtils.quoted_name('name'), :conditions => "owner_info_node_id is null") if item.nil? 
    return item
  end

  def prev_at_top
    item = Attribute.find(:first, :order => FtUtils.quoted_name('name') + ' DESC', :conditions => ["name < ? AND owner_info_node_id is null", self.name] )
    item = Attribute.find(:first, :order => FtUtils.quoted_name('name') + ' DESC', :conditions => "owner_info_node_id is null") if item.nil? 
    return item
  end

  def next_item
    return next_at_top() unless self.container_obj()
    item = self.container_obj().items.find(:first, :order => FtUtils.quoted_name('name'), :conditions => ["name > ? AND type = ?", self.name, "Attribute"] )
    item = self.container_obj().items.find(:first, :order => FtUtils.quoted_name('name'), :conditions => ["type = ?", "Attribute"]) if item.nil? 
    return item
  end

  def prev_item
    return prev_at_top() unless self.container_obj()
    item = self.container_obj().items.find(:first, :order => FtUtils.quoted_name('name') + ' DESC', :conditions => ["name < ? AND type = ?", self.name, "Attribute"] )
    item = self.container_obj().items.find(:first, :order => FtUtils.quoted_name('name') + ' DESC', :conditions => ["type = ?", "Attribute"]) if item.nil? 
    return item
  end

  def self.items_for_index
    return Attribute.find(:all, :order => 'name')
  end
  ...
  def self.default_query(cond=nil)
    ...
    return <<-EOS
          (SELECT attribute.name as #{FtUtils.quoted_name('name')},
              attribute.created_at as #{FtUtils.quoted_name('created_at')},
              attribute.updated_at as #{FtUtils.quoted_name('updated_at')},
              attribute.sort_code as #{FtUtils.quoted_name('sort_code')},
              attribute.value as #{FtUtils.quoted_name('value')},
              owner.name as #{FtUtils.quoted_name('owner')},
              owner.id as owner_info_node_id,
              attribute.type as type,
              attribute.id as id
          FROM
              #{FtUtils.quoted_name('items')} AS #{FtUtils.quoted_name('attribute')}
              left outer join #{FtUtils.quoted_name('items')} as #{FtUtils.quoted_name('owner')} on attribute.owner_info_node_id = owner.id
          WHERE 
  ...
  def self.query_specs
    ...
    @@query_specs = {
      "list_all" => 
        { :title => "List of "+"Attribute".pluralize,
          :sql  => "SELECT * FROM #{@@default_query}",
          :cols => Attribute.value_attrs.concat(Attribute.foreign_key_attrs),
  ...
  def get_attr_val(ft_col_name,ctx='html')
    attr_info = Attribute.attr_type_info(ft_col_name)
...
#BEGIN-UID.usermethods
   ...
   def qname
      q_name = iname()
      q_name = info_node.qname() + "/" + q_name if info_node
      return q_name
   end
