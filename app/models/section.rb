...
class Section < Item
   ...
   def get_children
    ...
    def self.attr_type_info(attr_name=nil)
        unless defined?(@@attr_info)
          ...
          @@attr_info['body'] = { :datatype => 'text', :modifier => '', :is_identity_attr => false, :default => '', :mode => 'rw' }
        end
    ...
    def self.value_attrs
      ["name","created_at","updated_at","sort_code","body"]
    end
 ...
 public
  ...
  def next_at_top
    item = Section.find(:first, :order => FtUtils.quoted_name('name'), :conditions => ["name > ? AND owner_info_node_id is null", self.name] )
    item = Section.find(:first, :order => FtUtils.quoted_name('name'), :conditions => "owner_info_node_id is null") if item.nil? 
    return item
  end

  def prev_at_top
    item = Section.find(:first, :order => FtUtils.quoted_name('name') + ' DESC', :conditions => ["name < ? AND owner_info_node_id is null", self.name] )
    item = Section.find(:first, :order => FtUtils.quoted_name('name') + ' DESC', :conditions => "owner_info_node_id is null") if item.nil? 
    return item
  end

  def next_item
    return next_at_top() unless self.container_obj()
    item = self.container_obj().items.find(:first, :order => FtUtils.quoted_name('name'), :conditions => ["name > ? AND type = ?", self.name, "Section"] )
    item = self.container_obj().items.find(:first, :order => FtUtils.quoted_name('name'), :conditions => ["type = ?", "Section"]) if item.nil? 
    return item
  end

  def prev_item
    return prev_at_top() unless self.container_obj()
    item = self.container_obj().items.find(:first, :order => FtUtils.quoted_name('name') + ' DESC', :conditions => ["name < ? AND type = ?", self.name, "Section"] )
    item = self.container_obj().items.find(:first, :order => FtUtils.quoted_name('name') + ' DESC', :conditions => ["type = ?", "Section"]) if item.nil? 
    return item
  end

  def self.items_for_index
    return Section.find(:all, :order => 'name')
  end
  ...
  def self.default_query(cond=nil)
    ...
    return <<-EOS
          (SELECT section.name as #{FtUtils.quoted_name('name')},
              section.created_at as #{FtUtils.quoted_name('created_at')},
              section.updated_at as #{FtUtils.quoted_name('updated_at')},
              section.sort_code as #{FtUtils.quoted_name('sort_code')},
              section.body as #{FtUtils.quoted_name('body')},
              owner.name as #{FtUtils.quoted_name('owner')},
              owner.id as owner_info_node_id,
              section.type as type,
              section.id as id
          FROM
              #{FtUtils.quoted_name('items')} AS #{FtUtils.quoted_name('section')}
              left outer join #{FtUtils.quoted_name('items')} as #{FtUtils.quoted_name('owner')} on section.owner_info_node_id = owner.id
          WHERE 
  ...
  def self.query_specs
    ...
    @@query_specs = {
      "list_all" => 
        { :title => "List of "+"Section".pluralize,
          :sql  => "SELECT * FROM #{@@default_query}",
          :cols => Section.value_attrs.concat(Section.foreign_key_attrs),
