class ValuesController < ItemsController
  ...
  def apply_to_checked_items
    if params[:action_to_perform] == 'Delete'    
        Value.destroy(params[:delchk].keys) if params[:delchk] && params[:delchk].keys.size > 0
    end 
  ...
  def replace_column_value
    ...
    Value.update_all("#{params[:field]} = '#{params[:new_value].strip}'", where_clause)
    redirect_to :back
  end
  ...
  def get_related
    ...
    if params[:relname].to_s.underscore == 'property_attrs' then
      render :xml => @value.property_attrs.find(:all)
      return
    end
  ...
  def create
    ...
    @new_obj = @value = Value.new(params[:value])
    ...
    if ! ok_to_add_or_update?
      ...
      respond_to do |format|
        if @value.save
          ...
          format.html {
            flash[:notice] = "#{@value.iname} was successfully created."
            redirected_to_aggregator = update_aggregator(@value,params)
            if ( ! redirected_to_aggregator ) && session[:post_create_redirect]
                url = session[:post_create_redirect] + "?value_id=" + @value.id.to_s + "&value_ids[]=" + @value.id.to_s 
                redirect_to url
                session[:post_create_redirect] = nil
            else 
                redirect_to :id => @value.id, :action => (params[:commit].to_s.include?("Another") ? 'new' : 'edit') if ! redirected_to_aggregator 
            end
          }
        else
          format.xml  { render :xml => @value.errors, :status => :unprocessable_entity }
          format.html {
            flash[:notice] = 'Value could not be created. See errors.'
            render :action => 'new'
          }
        end
      end
  ...
  def update
    @updated_obj = @value = Value.find(params[:id])
    sanitize_params(:value)
    ...
    if ! ok_to_add_or_update?
      ...
      @ft_subject = "Value"
      @ft_activity = "Edit"
      redirect_to :action => 'edit', :id => @value and return
    end 
      ...
      if @value.update_attributes(params[:value])
        ...
        format.html {
          ...
          redirect_to :action => 'show', :id => @value
        }
      else
        format.xml  { render :xml => @value.errors, :status => :unprocessable_entity }
        format.html { 
          redirect_to :action => 'edit', :id => @value
        }
      end
    end
  end
  ...
  def ok_to_add_or_update?
    if params[:value][:name].to_s.empty?
  ...
  def upload_csv
    ...
    #fn = "#{RAILS_ROOT}/uploads/value_data.csv"
    #File.open(fn, "wb") { |f| f.write(params[:csv_file].read) }
    #Value.load_csv_file(fn) if Value.respond_to? :load_csv_file

    Value.load_csv_str(params[:csv_file].read) if Value.respond_to? :load_csv_str
  ...
  def download_csv
    ...
    send_data csv_string, :filename => 'value_data.csv', :type => 'text/csv'
  end 
  ...
  def run_query
    ...
    @query = Value.query_specs[params[:query]]
    ...
    @count = Value.count_by_sql("SELECT COUNT(id) FROM (#{@base_sql}) AS row_data")
    
    @sql = @base_sql + (@sort_fields ? " ORDER BY #{@sort_fields}" : "") + (@page_clause ? " #{@page_clause}" : "") 
    @rows = Value.find_by_sql(@sql)

    @cols = @query[:cols] || (@rows.size > 0 && @rows[0].attributes.keys)

    @link_params = { :controller => 'values', :action => 'show' }
  ...
  def prepare_list
    ...
    @ft_subject  = ( @ft_subject || "Value".pluralize )
    ...
    adjust_sorting_and_filtering_params(Value,params)
